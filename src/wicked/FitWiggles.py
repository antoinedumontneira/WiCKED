...
from scipy.signal import find_peaks, savgol_filter  #  peak finder
...
def get_masked_regions(instance):
    ...
    if instance.nrs_detectors == 2:
        wave, gap_window, lines_to_be_flagged = (
            instance.wave,
            instance.gap_window,
            instance.lines_to_be_flagged,
        )
        allowed_wavelenghts = np.ones(wave.shape, bool)
        allowed_wavelenghts[(wave > gap_window[0]) & (wave < gap_window[1])] = (
            False  # exclude gap
        )
    else:
    ...
    for iltbf in range(len(lines_to_be_flagged)):
        allowed_wavelenghts[
            (wave > lines_to_be_flagged[iltbf][0])
            & (wave < lines_to_be_flagged[iltbf][1])
        ] = False
...
def power_law_model_fit(wave, flux, masked_regions, do_plots=False):
    ...
    ########### POWER LAW FUNCTION ######
    power_law = lambda x, a1, b1, c: b1 * x**a1 + c  ### POWER-LAW  # noqa
    ######################################
    ...
    for guess_i in slope_guesses:
        ...
        popt, pcov = curve_fit(
            ...
            method="trf",
            max_nfev=10000,
        )  # need to set max iterations to larger than default else fit fails erroneously
        A1s.append(popt[0])
        B1s.append(popt[1])
        Cs.append(popt[2])
...
def power_law_stellar_fit(
    ...
    if (
        ...
        )  ## smoothing template with a width of 1.1 microns
        smooth_data = savgol_filter(
            np.nan_to_num(data), int(len(data) / 2), 3
        )  ## smoothing template with a width of 1.1 microns

        def power_law_plus_annular_model(x, A, B, C, D, f1, f2, f3):
            total_model = (
                A * power_law_template_fit
                + B * spec_ref_in_fit
                + C * spec_ref_out_fit
                + D * smooth_data_fit
                + f1 * x**2
        ...
        guesses = [
            ...
            0.5,
            1.0,
            guesses_polynomial[0],
            guesses_polynomial[1],
            guesses_polynomial[2],
        ]  ## ASUME EUQAL CONTRIBUTION FROM BOTH TEMPLATES. We set the amplitud guess to 1.0
        popt, pcov = curve_fit(
            power_law_plus_annular_model,
            xdata=wave,
            ydata=data_fit,
            sigma=espec_fit,
            absolute_sigma=True,
        ...
        A_opt, B_opt, C_opt, D_opt, f1_opt, f2_opt, f3_opt = (
            popt[0],
            popt[1],
            popt[2],
            popt[3],
        ...
        power_law_flux_model = power_law_plus_annular_reconstruc(
    ...
    if smooth_model == False:
        ...
        guesses = [
            ...
            guesses_polynomial[1],
            guesses_polynomial[2],
        ]  ## ASUME EUQAL CONTRIBUTION FROM BOTH TEMPLATES
        popt, pcov = curve_fit(
            power_law_plus_annular_model,
            xdata=wave,
            ydata=data_fit,
        ...
        A_opt, B_opt, C_opt, d1_opt, d2_opt, d3_opt = (
            popt[0],
            popt[1],
            popt[2],
            popt[3],
        ...
        power_law_flux_model = power_law_plus_annular_reconstruc(
            wavelenght, A_opt, B_opt, C_opt, d1_opt, d2_opt, d3_opt
        )
...
def make_plots(dictionary, lines_to_be_flagged, gap_window, x, y):
    ...
    for iltbf in range(len(lines_to_be_flagged)):
        ax.axvspan(
            lines_to_be_flagged[iltbf][0],
            lines_to_be_flagged[iltbf][1],
            alpha=0.1,
            color="red",
        )
        cx.axvspan(
    ...
    ax.plot(
        wave, power_law_stellar_model, color="blue", label="Best-Fit model", alpha=0.8
    )
    ...
    cx.plot(
        wave,
        spec_corr - power_law_stellar_model,
        color="gray",
        label="Reesidual",
        alpha=1,
    )
    cx.plot(
        ...
        spec_corr,
        color="darkblue",
        label="Corrected single-pixel spectrum",
        alpha=1,
    )
...
def cos_fn_reconstruc2(p, x_model):
    ...
    y = Amp1 * np.cos(
        2 * np.pi * model_freq1 * x_model + np.pi / 2 * p[4]
    ) + Amp2 * np.sin(2 * np.pi * model_freq2 * x_model)
    return y
...
def loop_for_fit_wiggles(
    spec, wiggle_spectrum, espec, power_law_stellar_model, maxspec, args, iplotF=1
):
    ...
    while correct_px == "r":
        ...
        filter_spec = savgol_filter(
            wiggle_spectrum, 10, 3
        )  ### smothing of ~0.02 microns for finding peaks easily
        if nrs_detectors == 2:
            filter_spec[(wave > gap_window[0]) & (wave < gap_window[1])] = (
                0.0  # exclude gap
            )
        max_peaks, _ = find_peaks(
            filter_spec, distance=60, height=np.std(filter_spec) * 0.1
        )  ### peaks must be ~ 0.1 microns apart
        min_peaks, _ = find_peaks(
            filter_spec * -1, distance=60, height=np.std(filter_spec) * 0.1
        )
        combined_peaks = np.sort(np.concatenate([min_peaks, max_peaks]))
        if (
            f0 == 30
        ):  ## default prior is 30. This steps is skipped if users define it's own freq. prior
            f0 = 0.5 / (
        ...
        for iN in range(N_rep):
            ...
            if peak_selector == 0:
                min_random, max_random = (
                    np.min([(int(max_peaks[0]) - 2), 20]),
                    np.min([(wave.size - 1) - max_peaks[-1], 20]),
                )
                random_shift = np.random.randint(
                    -min_random, max_random
                )  ### RANDOMLY CHANGE THE INDEX OF THE PEAK BY ~0.04 MICRONS
                peaks = max_peaks + random_shift
            if peak_selector == 1:
                min_random, max_random = (
                    np.min([(int(min_peaks[0]) - 2), 20]),
                    np.min([(wave.size - 1) - min_peaks[-1], 20]),
                )
                random_shift = np.random.randint(
                    -min_random, max_random
                )  ### RANDOMLY CHANGE THE INDEX OF THE PEAK BY ~0.04 MICRONS
            ...
            if peak_selector == 2:
                min_random, max_random = (
                    np.min([(int(combined_peaks[0]) - 2), 20]),
                    np.min([(wave.size - 1) - combined_peaks[-1], 20]),
                )
                random_shift = np.random.randint(
                    -min_random, max_random
                )  ### RANDOMLY CHANGE THE INDEX OF THE PEAK BY ~0.04 MICRONS
            ...
            peaks = np.insert(
                peaks, 0, 0
            )  ### ADD FIRST ELEMENT, SO WE CAN FIT BETWEEN THE BEGINNING AND THE 1ST PEAK
            ...
            for w_i in np.arange(len(peaks)):
                if (w_i) == len(peaks) - 1:  ## FOR THE LAST PEAK IN THE SPECTRUM
                    if peaks[w_i] >= wave.size:  ## FOR THE LAST PEAK IN THE SPECTRUM
                        peaks[w_i] = (
                            wave.size - 60
                        )  #### IF THE RANDOM IND IS LARGER THAN THE WAVE, FIT IN A 60 PIX WINDOWN
                ...
                if nrs_detectors == 2:
                    flag_fit[(wave > gap_window[0]) & (wave < gap_window[1])] = (
                        False  # exclude gap
                    )
                for iltbf in range(len(lines_to_be_flagged)):
                    flag_mod[
                ...
                # ................................
                fa = {
                    "x_model": x_model[flag_mod],
                    "x": wave[flag_mod],
                    "y": wiggle_spectrum[flag_mod],
                    "err": espec[flag_mod],
                }
                m = mpfit.mpfit(
                    model, parinfo=parinfo, functkw=fa, ftol=1e-15, xtol=1e-15, quiet=1
                )
                if m.status <= 0:
                    # print ('error message = ', m.errmsg)
                    continue
                best_chi2_mod = chisquare(
                    m.params,
                    x_model[flag_mod],
                    wave[flag_mod],
                    wiggle_spectrum[flag_mod],
                    espec[flag_mod],
                ) / (wave[flag_mod].size - m.params.size)
                # repeat the fit, with random initializations for the initial parameters
                for k in range(30):  #### ORIGINALLY SET TO 60
                    pbin = 20
                    for i in range(len(p)):
                        p[i][0] = (
                            int(rn.uniform(1, pbin)) * (p[i][1] - p[i][2]) / pbin
                            + p[i][2]
                            + 1e-6
                        )
                    ...
                    chi2_mod_new = chisquare(
                        m0.params,
                        x_model[flag_mod],
                        wave[flag_mod],
                        wiggle_spectrum[flag_mod],
                        espec[flag_mod],
                    ) / (wave[flag_mod].size - m0.params.size)

                    if chi2_mod_new < best_chi2_mod:
                        m = m0
                        best_chi2_mod = chi2_mod_new

                    final_model[iN][flag_fit] = cos_fn_reconstruc(
                        m.params, x_model[flag_fit]
                    )
            ...
            if iN <= 1:  ## SET REFERENCE FOR A CHI2 FOR THE WHOLE WIGGLE SPECTRUM
                ...
                if (len(l_bins) > 0) & (loop_chi2 <= (best_chi2 + 0.01 * best_chi2)):
                    ...
                    if nrs_detectors == 2:
                        mask_freq_channels[
                            (sorted_l_bins > gap_window[0])
                            & (sorted_l_bins < gap_window[1])
                        ] = False
                    freq_par = np.polyfit(
                        sorted_l_bins[mask_freq_channels],
                        sorted_f_bins[mask_freq_channels],
                        3,
                    )
            ...
            if (iplotF == 0) & (f_walls == 2):
                if iN == 0:
                    ...
                    bx.plot(
                        wave[min_peaks],
                        wiggle_spectrum[min_peaks],
                        "X",
                        c="limegreen",
                        markersize=5,
                    )

        best_wiggle_model = savgol_filter(
            np.nanmedian(final_model, axis=0), 5, 3
        )  # np.nanmedian(final_model, axis=0)
        ...
        if (iplotF == 0) & (f_walls == 2):
            ...
            bx.text(
                0.05,
                0.93,
                "Median Freq. wiggles = " + meam_frequ_wiggle + r" [1/$\mu$]",
                ha="left",
                va="top",
            ...
            bx.plot(
                wave, best_wiggle_model, color="red", label="Wiggle Model", alpha=0.6
            )
            ...
            cx.plot(
                ...
                spec_corr,
                color="red",
                label="Corrected single-spaxel spectrum",
                alpha=0.9,
            )
            cx.legend(loc="upper right", prop={"size": 10}, mode="expand", ncol=2)
            plt.show(block=False)
        ...
        if correct_px == "y":
            if center_spec == "yes":
                ...
                return spec_corr * maxspec, {
                    "wave": wave,
                    "wiggles_spec": wiggle_spectrum,
                    "spec_ref_in": spec_ref_in,
                    "spec_ref_out": spec_ref_out,
                    "pw_law_stellar_model": power_law_stellar_model,
                    "original_spec": spec,
                    "corrected_spec": spec_corr,
                    "final_model": best_wiggle_model,
                }
    return
...
def fitwiggles(
    ...
    for i in range(len(affected_pixels)):
        ...
        if ~((ix == self.nuc_y) & (iy == self.nuc_x)):
            if smooth_spectrum == "y":
                wiggle_spectrum = savgol_filter(
                    spec - power_law_stellar_model, 10, 3
                )  ### APPLY A ~0.018 microns SMOOTHING TO MAKE WIGGLES MORE VISIBLE. ONLY RECOMMENDED FOR LOW-S/N.
            else:
                wiggle_spectrum = (
                    costume_sigmaclip(wave, spec) - power_law_stellar_model
                )  ######### SIGMA CLIPPING OUTLIERS IN SPECTRUM!! THE ORIGINAL SPECTRUM IS NOT ALTERED, IT IS ONLY FOR THE WIGGLE SPECTRUM
    ...
    if do_plots:
        for i in range(len(PLOTS)):
            make_plots(
                PLOTS[i], lines_to_be_flagged, gap_window, tasks[i][2], tasks[i][1]
            )
    ...
    for i in range(len(sorted_lf_bins)):
        hdu_01[1].data[:, sorted_lf_bins[i][1], sorted_lf_bins[i][0]] = sorted_lf_bins[
            i
        ][2]
