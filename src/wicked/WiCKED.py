...
class WICKED:
    ...
    def __init__(self, pathcube, cube_name, redshift):
        ...
        self.wave = hdu_01[1].header["CRVAL3"] + hdu_01[1].header["CDELT3"] * np.arange(
            0, hdu_01[1].header["NAXIS3"]
        )  # um
        ...
        if self.jwst_filter == "F100LP":
            if self.nrs_detectors == 2:
                ...
                self.good_continuum = [
                    [1.02, 1.42],
                    [1.5, 1.86],
                ]  ## Default, excludes only instrument gap and edges for the High-res data
        ...
        if self.nrs_detectors == 2:
            self.gap_mask = ~(
                (self.wave > self.gap_window[0]) & (self.wave < self.gap_window[1])
            )
        ...
        self.bfi = [
            0.5,
            50,
        ]  # boundaries for the fit for the wiggles frequency, to be used for the 1d spectrum with highest S/N
    ...
    def mask_lines_and_peaks(self, data, do_plots=False):
        ...
        peaks_abs = find_peaks_cwt(
            -data, min_snr=self.min_peak_snr, widths=self.peak_width
        )
        peaks_em = find_peaks_cwt(
            data, min_snr=self.min_peak_snr, widths=self.peak_width
        )
        try:
        ...
        if do_plots:
            ...
            zx1.vlines(
                list_em_lines, ymin=0.02, ymax=1, colors="r", label="given mask lines"
            )
    ...
    def setup_templates(
        ...
        for iltbf in range(len(self.good_continuum)):
            self.con_windows[
                (self.wave > self.good_continuum[iltbf][0])
                & (self.wave < self.good_continuum[iltbf][1])
            ] = True
        ...
        for iy in range(self.cube.shape[1]):
            for jx in range(self.cube.shape[2]):
                if (iy - nuc_y) ** 2 + (jx - nuc_x) ** 2 < in_radius**2:
                    single_spec = np.nan_to_num(
                        self.cube[:, iy, jx],
                        nan=np.nanmedian(self.cube[:, iy, jx][self.con_windows]),
                    )
                    median_spec = np.nanmedian(single_spec)
                    single_spec = (
                        costume_sigmaclip(self.wave, single_spec / median_spec)
                        * median_spec
                    )
                    spec_ref_in += single_spec
                if (
                    in_radius**2
                    <= (iy - nuc_y) ** 2 + (jx - nuc_x) ** 2
                    <= out_radius**2
                ):
                    single_spec = np.nan_to_num(
                        self.cube[:, iy, jx],
        ...
        if self.nrs_detectors == 2:
            ...
            maxspec_ref_in = np.nanmax(
                [
                    np.nanmax(spec_ref_in[self.wave < self.gap_window[0]]),
                    np.nanmax(spec_ref_in[self.wave > self.gap_window[1]]),
                ]
            )
            maxspec_ref_out = np.nanmax(
                [
                    np.nanmax(spec_ref_out[self.wave < self.gap_window[0]]),
                    np.nanmax(spec_ref_out[self.wave > self.gap_window[1]]),
                ]
            )
        else:
            WICKED.mask_lines_and_peaks(self, data=spec_ref_in)
        ...
        if do_plots:
            ...
            zx1.plot(
                self.wave,
                spec_ref_in,
                label="Inner Integrated Spectrum",
                c="k",
                alpha=1,
            )
            zx1.plot(
                self.wave,
                spec_ref_out,
                label="Outer Integrated Spectrum",
                c="orange",
                alpha=0.7,
            )
            for iltbf in range(len(self.lines_to_be_flagged)):
                zx1.axvspan(
                    xmin=self.lines_to_be_flagged[iltbf][0],
                    xmax=self.lines_to_be_flagged[iltbf][1],
                    ymin=0,
                    ymax=1,
                    color="red",
                    alpha=0.2,
            ...
            if self.nrs_detectors == 2:
                zx1.axvspan(
                    xmin=self.gap_window[0],
                    xmax=self.gap_window[1],
                    ymin=0,
                    ymax=1,
                    color="gold",
                    alpha=0.2,
    ...
    def clean_central_spaxel(self, nuc_x=None, nuc_y=None, N_rep=30, iplotF=0):
        ...
        spec = np.nan_to_num(
            self.cube[:, nuc_y, nuc_x],
            nan=np.nanmedian(self.cube[:, nuc_y, nuc_x][self.con_windows]),
        )
        ...
        if self.nrs_detectors == 2:
            maxspec = np.nanmax(
                [
                    np.nanmax(spec[wave < gap_window[0]]),
                    np.nanmax(spec[wave > gap_window[1]]),
                ]
            )
        else:
        ...
        espec = (
            np.nan_to_num(
                self.ecube[:, nuc_y, nuc_y],
                nan=np.nanmedian(self.ecube[:, nuc_y, nuc_y][self.con_windows]),
            )
            / maxspec
            * 2
        )
        espec[espec <= 0] = 1e-2
        if self.nrs_detectors == 2:
            power_law_stellar_model = power_law_stellar_fit(
                wave,
                spec,
                espec,
                spec_ref_in,
        ...
        corrected_central_spectrum, self.best_freq_par = loop_for_fit_wiggles(
            spec,
            spec_to_be_modelled,
            espec,
...
def meanclip(
    image, clipsig=3, maxiter=5, converge_num=0.02, verbose=False, returnSubs=False
):
    ...
    for i in range(maxiter):
        ...
        if ct > 0:
            subs = subs[wsm]
        if (
            (float(np.abs(ct - lastct)) / lastct <= converge_num)
            or (iter > maxiter)
            or (ct == 0)
        ):
            break
