...
def fit_wiggly_spaxels(freq_range,wave,data,masked_lines,con_windows,model_spaxel,detector, smooth_spectrum,do_plots=False):
    ...
    if smooth_spectrum == "yes":
        wiggles_spaxel = savgol_filter(wiggles_spaxel,10,3)   ### APPLY A ~0.018 microns SMOOTHING TO MAKE WIGGLES MORE VISIBLE. ONLY RECOMMENDED FOR LOW-S/N. 
    ...
    if do_plots:
        ...
        #ax1.plot(wave_for_model[masked_lines_for_model] ,0.06 +spec_ref_for_model[masked_lines_for_model] - np.nanmedian(spec_ref_for_model[masked_lines_for_model]),label="Smoothed Reference Spectrum",c="orange")
        if smooth_spectrum == "no":
            ax1.plot(wave_for_model ,(spec_for_model  - model_spaxel),label="Spectrum",c="royalblue")
        if smooth_spectrum == "yes":
            ax1.plot(wave_for_model ,wiggles_spaxel,label="Smoothed Spectrum",c="blue")
        ax1.vlines( wave_for_model[~masked_lines_for_model] ,np.nanmin(wiggles_spaxel) -0.1,np.nanmax(wiggles_spaxel)+0.1,alpha=0.05,color='red',label="Masked lines")
        ax1.set_xlabel(r'wavelenght [$\mu m$]')
        ax1.set_ylabel('Flux ')
        ax1.legend()
        ax2.set_title("Fourier Transform")
        ax2.plot(xf, 2.0/N * np.abs(yf_spaxel[0:N//2]) ,color='r',label='FFT of Spectrum')
        ax2.hlines(mean_ampl_large_freq ,min(xf),max(xf),color='green',label='reference median')
        ax2.hlines(one_sigma_level ,min(xf),max(xf),color='green',linestyles='dashed',label='1 sigma')
        ax2.hlines(spaxel_level ,min(xf),max(xf),color='red',linestyles='solid')
        ax2.axvspan(min_freq, max_freq, alpha=0.15, color='r',label="Wiggle regime")
        ax2.set_xlim(5,100)
        ax2.set_ylim(mean_ampl_large_freq-0.01,spaxel_level+0.02)
        ax2.set_xlabel(r'frequency [$\mu m^{-1}$]')
        ax2.legend()
    if spaxel_level >= 0:
        return spaxel_level / one_sigma_level
    else:
...
def get_wiggly_pixels(self,radius = 10, N_Cores = 1,detector = None,smooth_spectrum="no",do_plots=False):
    ...
    for i in range(len(power_law_stellar_models)):
        ...
        results.append( (pool.apply_async( fit_wiggly_spaxels ,(freq_range,wave,spec,masked_lines,self.con_windows,bestmodel_spaxel,detector,smooth_spectrum) ),y,x,spec,espec,maxspec,bestmodel_spaxel ) )
...
def plot_wiggle_FFT(self,X, Y,smooth_spectrum,detector = None):
    ...
    sigma_ratio = fit_wiggly_spaxels(freq_range,self.wave,spec,get_masked_regions(self),self.con_windows,best_model,detector,smooth_spectrum=smooth_spectrum,do_plots=True) 
